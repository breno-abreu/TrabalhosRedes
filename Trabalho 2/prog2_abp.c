  ...
  struct pkt packet;
{
  ...
  {
    printf("O pacote contendo o número de ACK foi corrompido! Necessário enviar o pacote original novamente.\n");
    printf("[A][REENVIANDO PACOTE]\n");
    A_output(A_msg_copia);
  }
}
  ...
  struct pkt packet;
{
  ...
  if(soma + packet.checksum == -1)
  {
    printf("A mensagem não foi corrompida. Tudo OK!\n");
    if(packet.seqnum == 0)
      B_acknum = 0;
  ...
  {
    printf("A mensagem foi corrompida! Necessário enviá-la novamente.\n");
    B_acknum = -1;
  }
